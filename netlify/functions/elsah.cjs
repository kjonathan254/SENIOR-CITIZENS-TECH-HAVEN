...
// NOTE: KB file lives next to this function (netlify/functions/elsah-kb.json)
// so it is bundled with the deployed function. A copy also exists at data/.
let knowledgeBase = null;
try {
  const kbCandidates = [
    path.join(__dirname, 'elsah-kb.json'),
    path.join(__dirname, '..', 'data', 'elsah-kb.json')
  ];
  const kbPath = kbCandidates.find((p) => fs.existsSync(p));
  if (kbPath) {
...
// Exported for unit testing (tests/rate-limit.test.cjs)
exports.checkRateLimit = checkRateLimit;
exports._rateLimitStore = rateLimitStore;
